...
SELECT city_name, state_name, city.area
FROM city
...
-- SQL does what it is told, which may not be sensible

-- SELECT *
-- FROM park
-- JOIN state ON park.area != state.area;
...
SELECT state_name, MIN(date_established) AS earliest_park
FROM state
FULL OUTER JOIN park_state ON state.state_abbreviation = park_state.state_abbreviation
FULL OUTER JOIN park ON park_state.park_id = park.park_id
GROUP BY state_name
ORDER BY state_name;
...
SELECT COUNT(*), 'City' AS kind_of_place
FROM city
WHERE city_name LIKE 'W%'
UNION
SELECT COUNT(*), 'State' AS kind_of_place
FROM state
WHERE state_name LIKE 'W%'
GROUP BY kind_of_place
ORDER BY kind_of_place;

-- ****** WARNING WARNING **** NEW CONTEXT ******
-- MovieDB
