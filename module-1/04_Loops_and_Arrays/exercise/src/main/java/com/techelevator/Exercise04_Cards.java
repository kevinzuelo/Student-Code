...
public class Exercise04_Cards {
    ...
    */
    ...
    public String[] discardFirstCard(String[] hand) {
        String[] newFourCardHand = new String[4];
        for (int i = 0; i < newFourCardHand.length; i++) {
            newFourCardHand[i] = hand[i + 1];
        }
        return newFourCardHand;
    }
    ...
    public String[] discardTopCard(String[] remainingDeck) {
        if(remainingDeck.length <= 1){
            ...
            String[] deckWithoutTopCard = new String[remainingDeck.length -1];
            for (int i = 1; i < remainingDeck.length; i++) {
                deckWithoutTopCard[i - 1] = remainingDeck[i];
            }
